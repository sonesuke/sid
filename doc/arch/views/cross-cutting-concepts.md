...
Technology selection SHALL prioritize **AI Proficiency** (accuracy of LLM code generation) and **Ecosystem Maturity** over human learning curves.
...
Authentication SHALL be delegated to an External Identity Provider (OIDC compliant).
...
Authorization SHALL use a Role + Scope model.
...
Business Errors (e.g., "Insufficient Funds") SHALL be distinguished from System Errors (e.g., "DB Connection Failed").
...
All errors SHALL follow standard JSON Problem Details format (RFC 7807).
...
A **Correlation ID** is MANDATORY for all internal and external requests. It MUST be propagated across service boundaries.
...
Zero Trust Model. Do not rely on network perimeter (VPC) for security.
...
TLS 1.2+ is MANDATORY for all data in transit, including internal service-to-service communication.
...
APIs SHALL follow REST principles and use JSON as the data format.

<a id="CC-API-002"></a>

### Rule [CC-API-002]

API versioning strategy SHALL avoid breaking changes (prefer backwards-compatible additions).
...
Deployment Artifacts SHALL be **Immutable** (Build Once, Deploy Many).
...
Database schema changes SHALL be backward compatible to support simultaneous execution of Blue and Green versions.

<a id="CC-OPS-004"></a>

### Rule [CC-OPS-004]

CI/CD Pipelines SHALL be defined as Code (**Pipeline as Code**).
